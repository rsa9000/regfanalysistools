/**
 * Walk over registry file (regf) tree and print as much data as possible
 *
...
static int regf_parse_raw(const char *pref, unsigned off, unsigned type,
			  unsigned len);
static int regf_parse_lst(const char *pref, unsigned off);
...
#define VLST_PR_WARN(__fmt, ...)	PR_WARN(off, "vlst", __fmt, ##__VA_ARGS__)
	...
	printf("%svals.{off,len} = 0x%08X, 0x%08X\n", pref, off, -rc->sz);
...
#define NK_PR_WARN(__fmt, ...)	PR_WARN(off, "nk", __fmt, ##__VA_ARGS__)
	...
	printf("%snk.{off,len} = 0x%08X, 0x%08X\n", pref, off, -rc->sz);
	...
	if (!cell) {
		...
		res = regf_parse_sk(__pref, nk->sec_off);
		if (res) {
	...
	if (nk->skey_num) {
		...
		if (!cell) {
			...
			res = regf_parse_lst(__pref, nk->skey_off);
			if (res) {
...
static int regf_parse_sk(const char *pref, unsigned off)
{
	...
	printf("%ssk.{off,len} = 0x%08X, 0x%08X\n", pref, off, -rc->sz);
...
#define VK_PR_WARN(__fmt, ...)	PR_WARN(off, "vk", __fmt, ##__VA_ARGS__)
	...
	printf("%svk.{off,len} = 0x%08X, 0x%08X\n", pref, off, -rc->sz);
	...
	snprintf(__pref, sizeof(__pref), "%sval.", pref);
	...
	if (vk->sz & 0x80000000) {	/* Embedded data */
		...
		res = regf_print_data(__pref, &vk->off, vk->type,
				      vk->sz & ~0x80000000);
		if (res) {
	...
	if (vk->sz <= 16344 || st.version < VER_1_5) {
		...
		res = regf_parse_raw(__pref, vk->off, vk->type, vk->sz);
		if (res) {
...
#define LF_PR_WARN(__fmt, ...)	PR_WARN(off, "lf", __fmt, ##__VA_ARGS__)
	...
	for (i = 0; i < lst->nelem; ++i) {
		...
		cache_cell_updref(cell, CT_NK);
		res = regf_parse_nk(pref, elems[i].off);
		if (res) {
...
#define RI_PR_WARN(__fmt, ...)	PR_WARN(off, "ri", __fmt, ##__VA_ARGS__)
	...
	for (i = 0; i < lst->nelem; ++i) {
		...
		cache_cell_updref(cell, CT_LST);
		res = regf_parse_lst(pref, elems[i].off);
		if (res) {
...
static int regf_parse_lst(const char *pref, unsigned off)
{
	...
	printf("%s%.2s.{off,len} = 0x%08X, 0x%08X\n", pref, lst->sign, off, -rc->sz);

	if (strncmp(lst->sign, "lf", 2) == 0) {
		return regf_parse_lst_lf(pref, off);
	} else if (strncmp(lst->sign, "lh", 2) == 0) {
		return regf_parse_lst_lh(pref, off);
	} else if (strncmp(lst->sign, "li", 2) == 0) {
		return regf_parse_lst_li(pref, off);
	} else if (strncmp(lst->sign, "ri", 2) == 0) {
		return regf_parse_lst_ri(pref, off);
	} else {
...
#define SLST_PR_WARN(__fmt, ...)PR_WARN(off, "slst", __fmt, ##__VA_ARGS__)
	...
	printf("%ssegm.{off,len} = 0x%08X, 0x%08X\n", pref, off, -rc->sz);
...
#define DB_PR_WARN(__fmt, ...)	PR_WARN(off, "db", __fmt, ##__VA_ARGS__)
	...
	printf("%sdb.{off,len} = 0x%08X, 0x%08X\n", pref, off, -rc->sz);
	printf("%sdb.nsegm = %u\n", pref, db->nsegm);
